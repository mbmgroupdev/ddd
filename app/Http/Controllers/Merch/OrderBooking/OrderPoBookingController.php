...
class OrderPoBookingController extends Controller
{
    ...
    {
        try {
            $orderList = DB::table('mr_order_entry as a')->whereNotIn('a.order_status',['Completed','Inactive'])->pluck('order_code','order_id')->toArray();
    ...
    public function getPoOrderListData()
    {
      ...
      return DataTables::of($data)->addIndexColumn()
          ...
          // ->addColumn('orderPo', function($data){
          //     $poDetails = DB::table('mr_po_booking_detail')->where('mr_po_booking_id',$data->id)->get()->toArray();
          //     $poDs = [];
          //     $poText = '';
          //     if(!empty($poDetails)) {
          //       foreach($poDetails as $poDetail) {
          //           $poDs[$poDetail->mr_order_entry_order_id][$poDetail->mr_purchase_order_po_id] = $poDetail->mr_purchase_order_po_id;
          //       }
          //     }
          //     if(!empty($poDs)) {
          //         $poText = '';
          //         foreach($poDs as $orderId=>$poIds) {
          //             $orderName = DB::table('mr_order_entry')->where('order_id',$orderId)->first();
          //             $poName = '';
          //             if(!empty($orderName)) {
          //                 foreach($poIds as $poId) {
          //                 $poInfo = DB::table('mr_purchase_order')->where('po_id',$poId)->first();
          //                 if(!empty($poInfo)) {
          //                     $poName .= $poInfo->po_no.'|';
          //                 }
          //             }
          //             $poName = substr($poName,0,-1);
          //             $poText .= '<span class="label label-info arrowed-right arrowed-in polisttitle" data-toggle="tooltip" title="'.$poName.'">'.$orderName->order_code.'</span> <br>';
          //         }
          //       }
          //     }
          //     return $poText;
          ...
          ->addColumn('action', function($data){
              ...
              // $mrBooking = MrOrderBooking::where('mr_po_booking_id',$data->id)->get();
              if(!$data->orderBooking->isEmpty()){
                  $action_buttons .= "
