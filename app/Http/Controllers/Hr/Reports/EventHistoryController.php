...
class EventHistoryController extends Controller
{
    ...
    public function listData()
    {
    	...
    	return DataTables::of($data)->addIndexColumn()
                            ->addColumn('modified_event', function($data){
                                $modified_events=json_decode($data->modified_event);
                                $view='';
                                if(isset($modified_events->unit) || isset($modified_events->hr_unit)) {
                                    $unit = isset($modified_events->unit)?isset($modified_events->unit):isset($modified_events->hr_unit);
                                    $unit_name=Unit::where('hr_unit_id',$unit)->first();
                                     // dd($unit_name);exit;
                                    if($data->type==1)
                                        {$view= "<div>  
